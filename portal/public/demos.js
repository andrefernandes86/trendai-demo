/**
 * Tool catalog for TrendAI Tools.
 *
 * Every tool is served on the SAME origin as the portal, under its `path`
 * (e.g. https://trendai.secnerd.io/v1fs). The portal reverse-proxies each path
 * to the tool container and requires a valid login session first, so a single
 * Cloudflare hostname fronts everything and nothing is reachable unauthenticated.
 *
 * `path`     must match a prefix registered in portal/server.js (DEMOS).
 * `category` groups the card under a side-menu section:
 *            'Demo Tools' | 'Business Tools' | 'Other'.
 */
window.TRENDAI_DEMOS = [
  {
    id: 'v1fs',
    category: 'Demo Tools',
    path: '/v1fs',
    title: 'TrendAI Vision One File Security',
    subtitle: 'Use Case · Scan File Servers with the SDK',
    icon: 'shield',
    desc: 'Use case: scan on-premises file servers and network shares for malware with the TrendAI Vision One File Security SDK. Point it at a mounted share, run an on-demand scan, and download a PDF report of any detections.',
    steps: [
      'Click Open to launch the TrendAI Vision One File Security scanner.',
      'Open Settings and enter your TrendAI Vision One File Security API key and region, then save.',
      'Browse to a mounted file-server path / network share (or the bundled EICAR test sample) and start a scan.',
      'Watch detections stream in, then download the PDF report for that file server.',
      'Talking point: scan file servers at scale via the SDK — only file hashes/metadata leave the environment, never the files themselves.',
    ],
    tags: ['TrendAI Vision One', 'File Security SDK', 'File Servers'],
  },
  {
    id: 'app-sec-file-sec',
    category: 'Demo Tools',
    path: '/appsec',
    title: 'TrendAI Vision One AI Guard & File Security',
    subtitle: 'Use Case · Securing an AI Chatbots',
    icon: 'file',
    desc: 'Use case: an AI chatbot that lets users chat and upload files. Two TrendAI Vision One modules protect it — AI Guard inspects every prompt for injection/abuse, and File Security scans every uploaded file for malware before it is accepted.',
    steps: [
      'Click Open to launch the AI chatbot. Ollama is bundled and pre-selected (http://ollama:11434).',
      'In Settings, enter your TrendAI Vision One API keys for AI Guard and File Security, then save.',
      'Chat normally to show the assistant responding via the local model.',
      'Click "Test Injection" (or type a jailbreak/prompt-injection) — AI Guard flags and blocks the malicious prompt.',
      'Upload a benign file (clean) then the EICAR sample — File Security detects and blocks the malicious upload.',
      'Talking point: one platform guards both the prompt layer and the file layer of a GenAI app.',
    ],
    tags: ['AI Guard', 'File Security SDK', 'Prompt Injection'],
    note: 'Bundled Ollama (gemma2:2b, CPU) is pre-wired at http://ollama:11434. Enter your TrendAI Vision One API keys for AI Guard and File Security in Settings to enable detection.',
  },
  {
    id: 'workload-protection',
    category: 'Demo Tools',
    path: '/workload',
    title: 'TrendAI Vision One — Server & Workload Protection',
    subtitle: 'Use Case · Agent-Based Workload Defense',
    icon: 'server',
    desc: 'Use case: a protected server workload under attack. Simulates an attacker exploiting a command-injection vulnerability — a real malware download, a real outbound connection, a real exploit request, real file tampering, and real log activity — and shows all 5 protection modules independently catching it. If a real agent protects this host, its own Vision One console shows the matching detections.',
    steps: [
      'Click Open to view the protected workload dashboard (all 5 modules start "Protected").',
      'Trigger Anti-Malware — writes a real EICAR test file to the workload; a real agent quarantines it (check Vision One).',
      'Trigger Web Reputation — makes a real outbound connection to Trend Micro\'s own WRS test domains, rotating Dangerous → Normal (check Vision One).',
      'Trigger Host IPS — sends a real network request over the wire, cycling through 3 SQL-injection rules and 4 browser exploit PoCs, all real TippingPoint rule IDs (check Vision One).',
      'Trigger Integrity Monitoring and Log Inspection — real file tampering and real log lines on this workload; these two need a one-time policy rule added in Vision One first (see README-REAL-DETECTION.md).',
      'Talking point: defense in depth — one exploited vulnerability, five independent modules each catching a different stage of the attack, visible in the real product console.',
    ],
    tags: ['Anti-Malware', 'Host IPS', 'Integrity Monitoring'],
    note: 'Every trigger produces real, host-visible activity (a real file write, a real network request) — not a pure simulation. Integrity Monitoring and Log Inspection additionally require a one-time rule added in the Vision One console to register there (Deep Security agents don\'t watch arbitrary paths by default) — see tools/workload-protection/README-REAL-DETECTION.md.',
  },
  {
    id: 'ai-scanner',
    category: 'Demo Tools',
    path: '/aiscanner',
    title: 'TrendAI Vision One — AI Scanner',
    subtitle: 'Use Case · Red-Teaming a Local LLM',
    icon: 'scan',
    desc: 'Use case: mirrors the real `tmas aiscan llm` workflow — point at a live model, run a battery of adversarial tests, and review findings mapped to the OWASP Top 10 for LLM Applications. Every test sends a real prompt to a real local Ollama model and analyzes the real response; nothing is scripted.',
    steps: [
      'Click Open, pick a local model (1B–4B parameters, pulled into the shared Ollama), and review the risk categories (all checked by default for the most comprehensive scan).',
      'Click "Run Full Scan" and watch each test run live — prompt injection, system-prompt leakage, sensitive-data fabrication, unsafe output handling, excessive agency, misinformation, and unbounded consumption.',
      'Review the findings table: OWASP LLM ID, severity, and a sanitized excerpt of the model\'s actual response as evidence.',
      'Talking point: two OWASP LLM categories (Data/Model Poisoning, Vector & Embedding Weaknesses) are honestly excluded — they need white-box access this black-box test can\'t provide.',
      'The real product goes further: the Settings panel accepts a Vision One API key for workflow parity, but this demo\'s engine runs entirely locally — point out the "Learn about AI Scanner" link for where the real console picks up.',
    ],
    tags: ['AI Scanner', 'OWASP LLM Top 10', 'Ollama'],
    note: 'An original, from-scratch adversarial-testing engine — not a copy of any third-party jailbreak toolkit. Every finding comes from a genuine prompt/response pair against the selected local model.',
  },
  {
    id: 'v1-chatbot',
    category: 'Demo Tools',
    path: '/v1chat',
    title: 'TrendAI Vision One — Chatbot',
    subtitle: 'Use Case · Conversational Vision One via MCP',
    icon: 'chat',
    desc: 'A chat interface in front of the real Trend Vision One MCP server (github.com/andrefernandes86/trendai-vision-one-mcp-server), built from source at image-build time. Every tool call is a genuine MCP tools/call over stdio to the real Go binary, which makes a real HTTPS call to the Trend Vision One API — Cloud Posture, IAM, Attack Surface Risk Management (CREM), Cloud Account Management, Email Security, Container Security, Endpoint Security, AI Security Guardrails, Cloud Risk Management, Workbench alerts, and Threat Intelligence. Nothing about the Vision One data shown is fabricated.',
    steps: [
      'Click Open, enter a Trend Vision One API key and region under "Vision One Connection", and Save & Connect.',
      'Pick an LLM under "Model Endpoint" — the bundled local Ollama, or any external OpenAI-compatible endpoint with tool/function-calling support — and Save.',
      'Browse the live "Vision One Capabilities" panel to see every tool the connected tenant actually exposes, grouped by category.',
      'Ask a question in the chat (e.g. "List the 10 most recent Workbench alerts" or "What endpoints have the highest risk score?") — watch the tool-call cards show exactly which Vision One API was hit, with what arguments, and the real result.',
      'Talking point: the server runs `-readonly=true` by default (upstream\'s own safety default) — flipping the Write Mode toggle enables genuinely destructive actions (deleting IAM accounts/API keys, threat intel objects, etc.) and requires an explicit opt-in.',
    ],
    tags: ['Vision One', 'MCP', 'Chatbot', 'Agentic'],
    note: 'The chat backend is original code; the tool-calling surface it drives is the real, unmodified upstream MCP server compiled straight from its GitHub source on every image build.',
  },
  {
    id: 'smish',
    category: 'Other',
    path: '/smish',
    title: 'Smish Detector',
    subtitle: 'Smishing Awareness · Education',
    icon: 'phone',
    desc: 'A security-awareness demonstration of SMS phishing ("smishing"): shows what data a malicious link can silently collect from a device — for training and privacy awareness only.',
    steps: [
      'Click Open to launch the smishing landing page.',
      'Share the demo link with a test device you control.',
      'Open the link on that device and let it collect the browser / device fingerprint.',
      'Go to the Report view and enter the report password (default: sms) to see the captured data.',
      'Talking point: how easily a single tapped link exposes device details — reinforce user training.',
    ],
    tags: ['Awareness', 'Smishing', 'Educational Use Only'],
    note: 'Educational use only, in controlled environments.',
  },
  {
    id: 'siemcalc',
    category: 'Business Tools',
    path: '/siemcalc',
    title: 'TrendAI Agentic SIEM Sizing Calculator',
    subtitle: 'Sizing · Credit Calculator',
    icon: 'calc',
    desc: 'A 6-phase sizing wizard that calculates annual credit requirements for TrendAI Vision One — Agentic SIEM. Walks through log ingestion, endpoint/cloud, email/network, and CREM risk events, with a live totals sidebar and a year-by-year breakdown.',
    steps: [
      'Click Open to launch the sizing wizard.',
      'Work through the 6 phases, entering unit counts, GB/day, and retention periods.',
      'Watch the live totals sidebar update credits as you type.',
      'On the Results step, review the grand total and year-by-year table.',
      'Export a clean, print-ready PDF report to share with the customer.',
    ],
    tags: ['Agentic SIEM', 'Sizing', 'Credits'],
  },
];
