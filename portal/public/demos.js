...
window.TRENDAI_DEMOS = [
  ...
  },
  {
    ...
    icon: 'server',
    desc: 'Use case: a protected server workload under attack. Simulates an attacker exploiting a command-injection vulnerability — attempting a malware download, exploit payloads, unauthorized file changes, and suspicious log activity — and shows all 5 protection modules independently catching it.',
    steps: [
      'Click Open to view the protected workload dashboard (all 5 modules start "Protected").',
      'Trigger Anti-Malware — simulates a command-injection malware download; the EICAR test signature is detected and quarantined.',
      'Trigger Web Reputation — simulates the outbound connection to the malware source being blocked by category.',
      'Trigger Host IPS — cycles through real SQL-injection exploit signatures with their actual TippingPoint rule IDs.',
      'Trigger Integrity Monitoring and Log Inspection — shows the attacker\'s file tampering and brute-force login pattern both getting flagged.',
      'Talking point: defense in depth — one exploited vulnerability, five independent modules each catching a different stage of the attack.',
    ],
    tags: ['Anti-Malware', 'Host IPS', 'Integrity Monitoring'],
    note: 'All detections run locally, entirely in memory (EICAR signature, real TippingPoint SQLi rule payloads) — nothing is written to disk and no live external requests happen at demo time.',
  },
  {
    id: 'smish',
    category: 'Other',
